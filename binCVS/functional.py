...
def command_init():
    if not os.path.exists(my_cvs.way_to_cvs):
        ...
        create_file(my_cvs.way_to_track_file)
...
def command_add(file_name):
    ...
    if len(new_track_files) > 0:
        write_file(my_cvs.way_to_track_file, new_track_files)
...
def command_commit(value):
    ...
    for t_file in my_cvs.track_files:
        if t_file not in my_cvs.added_files:
            ...
            write_file(my_cvs.way_to_added_file, t_file)
...
def command_delete(file_name):
    ...
    rewrite_file(my_cvs.way_to_track_file, track_files)
    ...
    if not len(folders) == 0:
        for folder in folders:
            if os.path.isdir(os.path.join(my_cvs.way_to_prj_ver, folder)):
                log += 'Версия : ' + folder + '\n'
                log += 'Дата: ' + str(read_file(os.path.join(my_cvs.way_to_prj_ver, folder, '.date'))[0]) + '\n'
                log += 'Комментарий: ' + str(read_file(os.path.join(my_cvs.way_to_prj_ver, folder, '.comment'))[0]) + '\n'
                log += '\n'
    else:
...
def add_track_files(files):
    ...
    write_file(my_cvs.way_to_track_file, added_files)
